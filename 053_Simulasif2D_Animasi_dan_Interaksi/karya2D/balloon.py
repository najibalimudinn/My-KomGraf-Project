...
class Balloon:
    ...
    def rotate(self, degree):
        for i in range(len(self.parts)):
            self.tms[i] = primitif.transformasi.rotate2D(degree, self.xc, self.yc + self.ry + self.a + self.ry*2, self.tms[i])
    
    def rotate_rubber(self, degree):
        for i in range(len(self.parts)):
            self.tms[i] = primitif.transformasi.rotate2D(degree, self.xc, self.yc, self.tms[i])
    
    def scale(self, sx, sy):
        for i in range(len(self.parts)):
            self.tms[i] = primitif.transformasi.scale2D(sx, sy, self.xc, self.yc + self.ry, self.tms[i])
    ...
    def draw(self):
        for color, part in zip(self.part_colors, self.parts):
            primitif.basic.draw_bentuk(part, color)